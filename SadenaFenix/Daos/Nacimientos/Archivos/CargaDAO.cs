...
namespace SadenaFenix.Daos.Nacimientos.Archivos
{
    public class CargaDAO : DataContext
    {
        ...
        private const string CONSULTA_FUENTE_SINAC = "SELECT FOLIO, NOMBRE, PATERNO, MATERNO, IIF(VAL(CEDOCVE) > 0 AND ISNUMERIC(ENT_NACM), VAL(ENT_NACM), 99), MPO_NAC, FECH_NACH, HORA_NACH, EDADM, EDOCIVIL, CALLE_RES, NUMEXT_RES, NUMINT_RES, ENT_RES, MPO_RES, LOC_RES, NOMASEN_RES, IIF(NUM_NACVIVO IS NULL, 0,NUM_NACVIVO),  IIF(VAL(NIV_ESCOL) > 1, VAL(NIV_ESCOL), 1), OCUPHAB, val(SEXOH) FROM NACIMIENTO";
